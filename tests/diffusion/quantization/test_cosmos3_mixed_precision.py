...
def test_block_cache_double_buffers_generation_blocks() -> None:
    ...
    for state in states:
        ...
        base = _BaseMethod()
        strategy.validate_before_processing(base, layer, state.module_name)
        base.process_weights_after_loading(layer)
        strategy.validate_after_processing(layer, state.module_name)
        layers.append(layer)
